  ...
  return (
    ...
    >
      ...
      <div className="flex-1 py-5 overflow-y-auto" style={{ opacity: isOpen ? 1 : 0, transition: 'opacity 0.2s', minWidth: '250px' }}>
        {navItems.map((item) => {
          ...
          return (
            <button
              key={item.id}
              className={`flex items-center px-5 py-3 mx-3 my-1 rounded-lg text-left w-[calc(100%-24px)] transition-all duration-200 font-medium ${
                isActive 
      ...
      <div className="p-5 border-t border-gray-200 shrink-0" style={{ opacity: isOpen ? 1 : 0, transition: 'opacity 0.2s', minWidth: '250px' }}>
        <button 
          className={`flex items-center px-5 py-3 mx-0 rounded-lg text-left w-full transition-all duration-200 font-medium ${
            activeItem === 'settings'
              ? 'bg-[#FDF1EA] text-brand-orange' 
              : 'text-gray-500 hover:bg-gray-100 hover:text-brand-blue'
          }`}
