...
const mockClients = [
  { id: "C001", name: "Ahmed Mohamed", phone: "0612345678", address: "Rue Mohammed V, Casablanca" },
  { id: "C002", name: "Fatima Ali", phone: "0623456789", address: "Quartier Hassan, Rabat" },
  { id: "C003", name: "Youssef Benali", phone: "0661234567", address: "Bd Zerktouni, Casablanca" },
  { id: "C004", name: "Khadija Mansouri", phone: "0698765432", address: "Av. Mohammed VI, Marrakech" },
  { id: "C005", name: "Omar Tazi", phone: "0677889900", address: "Quartier Industriel, Fès" },
  { id: "C006", name: "Salma El Amrani", phone: "0655443322", address: "Rue Hassan II, Tanger" },
];
...
const moroccanCities: Record<string, string[]> = {
  ...
  "Tanger-Tétouan-Al Hoceïma": ["Tanger", "Tétouan", "Al Hoceïma", "Larache", "Asilah"],
...
const DELIVERY_TYPES = [
  { id: "agence",   label: "À l'agence",  icon: Truck, pricePerKg: 20, description: "Client récupère à l'agence" },
  { id: "domicile", label: "À domicile",  icon: Home,  pricePerKg: 30, description: "Livraison à l'adresse" },
];



interface ClientInfo { name: string; phone: string; address: string; }
interface DestinatairInfo extends ClientInfo { country: string; city: string; region: string; }
...
function ClientSearch({ onSelect }: { onSelect: (c: typeof mockClients[0]) => void }) {
  const [query, setQuery] = useState("");
  const [open, setOpen] = useState(false);
  const ref = useRef<HTMLDivElement>(null);

  const filtered = query.trim()
    ? mockClients.filter((c) => c.name.toLowerCase().includes(query.toLowerCase()) || c.phone.includes(query))
    : [];
...
function ImageSlot({ index, file, onUpload }: { index: number; file: File | null; onUpload: (i: number, f: File) => void }) {
  ...
  return (
    <div
      ...
      className={`flex flex-col items-center justify-center w-16 h-16 rounded-lg border-2 border-dashed cursor-pointer transition-all ${
        file ? "border-emerald-400 bg-emerald-50" : "border-gray-200 bg-gray-50 hover:border-brand-orange hover:bg-[#FDF1EA]"
      }`}
    >
      {file ? (
        <div className="flex flex-col items-center gap-0.5">
          <Image size={14} className="text-emerald-500" />
          <span className="text-[9px] text-emerald-600 font-bold">IMG{index + 1} ✓</span>
        </div>
      ) : (
        <div className="flex flex-col items-center gap-0.5">
      ...
      <input ref={ref} type="file" accept="image/*" className="hidden" onChange={(e) => e.target.files?.[0] && onUpload(index, e.target.files[0])} />
    </div>
  );
}
...
const inputCls = "w-full px-3 py-1.5 bg-gray-50 border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-brand-orange focus:bg-white transition-all text-gray-700 placeholder-gray-300";



export default function AddColisPage({ onBack, onSave }: AddColisPageProps) {
  const today = new Date().toLocaleDateString("fr-FR");
  const [trackingNo, setTrackingNo] = useState(() => `C${Math.floor(100000 + Math.random() * 900000)}`);

  // Expéditeur
  const [sender, setSender] = useState<ClientInfo>({ name: "", phone: "", address: "" });
  const [saveSender, setSaveSender] = useState(false);

  // Destinataire
  const [receiver, setReceiver] = useState<DestinatairInfo>({ name: "", phone: "", address: "", country: "Maroc", city: "", region: "" });
  ...
  // Delivery type
  const [deliveryType, setDeliveryType] = useState<"agence" | "domicile">("domicile");
  ...
  const totalPrice = typeof weight === "number" && weight > 0 ? weight * pricePerKg : 0;
  const availableCities = receiver.region ? moroccanCities[receiver.region] ?? [] : [];

  const handleImageUpload = (idx: number, file: File) => {
    setImages((prev) => prev.map((f, i) => (i === idx ? file : f)));
  };

  const isValid = sender.name.trim() && receiver.name.trim() && typeof weight === "number" && weight > 0 && trackingNo.trim() !== "";
  ...
  const handleSave = () => {
    ...
    Swal.fire({ title: "Ajouté !", text: "Colis enregistré avec succès.", icon: "success", timer: 1400, showConfirmButton: false });
    onBack();
  };
    ...
    <div className="flex flex-col gap-5 w-full">
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="hidden md:flex items-center gap-2">
          ...
          >
            <Printer size={14} /> Enregistrer & Imprimer
          </button>
          <button
            onClick={handleSave}
            disabled={!isValid}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
        ...
        <div className="flex flex-col gap-5">
          ...
          <Section icon={User} title="Expéditeur">
            <ClientSearch onSelect={(c) => setSender({ name: c.name, phone: c.phone, address: c.address })} />
            <div className="grid grid-cols-2 gap-3">
              <Field label="Nom" required>
                <input type="text" value={sender.name} onChange={(e) => setSender((p) => ({ ...p, name: e.target.value }))} placeholder="Nom complet" className={inputCls} />
              </Field>
              <Field label="Téléphone">
                <input type="tel" value={sender.phone} onChange={(e) => setSender((p) => ({ ...p, phone: e.target.value }))} placeholder="0612345678" className={inputCls} />
              </Field>
            </div>
            <Field label="Adresse">
              <input type="text" value={sender.address} onChange={(e) => setSender((p) => ({ ...p, address: e.target.value }))} placeholder="Adresse complète" className={inputCls} />
            </Field>
            <label className="flex items-center gap-2 cursor-pointer w-fit">
              <input type="checkbox" checked={saveSender} onChange={(e) => setSaveSender(e.target.checked)} className="w-3.5 h-3.5 rounded accent-brand-orange cursor-pointer" />
              <span className="text-xs text-gray-500">Sauvegarder comme client</span>
            </label>
          </Section>

          {/* DESTINATAIRE */}
          <Section icon={MapPin} title="Destinataire">
            <ClientSearch onSelect={(c) => setReceiver((p) => ({ ...p, name: c.name, phone: c.phone, address: c.address }))} />
            <div className="grid grid-cols-2 gap-3">
              <Field label="Nom" required>
                <input type="text" value={receiver.name} onChange={(e) => setReceiver((p) => ({ ...p, name: e.target.value }))} placeholder="Nom complet" className={inputCls} />
              </Field>
              <Field label="Téléphone">
                <input type="tel" value={receiver.phone} onChange={(e) => setReceiver((p) => ({ ...p, phone: e.target.value }))} placeholder="0623456789" className={inputCls} />
              </Field>
            </div>
            <div className="grid grid-cols-3 gap-3">
              <Field label="Pays">
                <div className="relative">
                  <select value={receiver.country} onChange={(e) => setReceiver((p) => ({ ...p, country: e.target.value }))} className={inputCls + " appearance-none pr-7 cursor-pointer"}>
                    <option>Maroc</option><option>France</option><option>Espagne</option><option>Belgique</option><option>Canada</option>
                  </select>
              ...
              <Field label="Région">
                <div className="relative">
                  <select value={receiver.region} onChange={(e) => setReceiver((p) => ({ ...p, region: e.target.value, city: "" }))} className={inputCls + " appearance-none pr-7 cursor-pointer"}>
              ...
              <Field label="Ville">
                <div className="relative">
                  <select value={receiver.city} onChange={(e) => setReceiver((p) => ({ ...p, city: e.target.value }))} disabled={!availableCities.length} className={inputCls + " appearance-none pr-7 cursor-pointer disabled:opacity-50"}>
                    <option value="">Sélectionner...</option>
            ...
            <Field label="Adresse complète">
              <input type="text" value={receiver.address} onChange={(e) => setReceiver((p) => ({ ...p, address: e.target.value }))} placeholder="Quartier, Immeuble, Appartement..." className={inputCls} />
            </Field>
            <label className="flex items-center gap-2 cursor-pointer w-fit">
              <input type="checkbox" checked={saveReceiver} onChange={(e) => setSaveReceiver(e.target.checked)} className="w-3.5 h-3.5 rounded accent-brand-orange cursor-pointer" />
              <span className="text-xs text-gray-500">Sauvegarder comme client</span>
            </label>
          </Section>
        </div>
        ...
        <div className="flex flex-col gap-5">
          ...
          {/* DÉTAILS DU COLIS */}
          <Section icon={Scale} title="Détails du Colis">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
              <Field label="N° de Suivi (Tracking)" required>
                <div className="flex gap-2">
                  <input
                    type="text"
                    value={trackingNo}
                  ...
                  <button
                    type="button"
                    onClick={() => setTrackingNo(`C${Math.floor(100000 + Math.random() * 900000)}`)}
            ...
            <Field label="Photos (max 3)">
              <div className="flex items-center gap-2">
                {images.map((file, idx) => <ImageSlot key={idx} index={idx} file={file} onUpload={handleImageUpload} />)}
                <span className="text-[10px] text-gray-400 ml-1">Cliquez pour uploader</span>
              </div>
            </Field>
          </Section>

          {/* TYPE DE LIVRAISON */}
          <Section icon={Truck} title="Type de Livraison" badge="Requis">
            <div className="grid grid-cols-2 gap-3">
              {DELIVERY_TYPES.map((dt) => {
                ...
                return (
                  <button
                    key={dt.id}
                    type="button"
                    onClick={() => setDeliveryType(dt.id as "agence" | "domicile")}
                  ...
                  >
                    <div className={`mt-0.5 p-2 rounded-lg shrink-0 ${sel ? "bg-brand-orange text-white" : "bg-gray-200 text-gray-500"}`}>
                      <Icon size={14} />
                    </div>
                    <div>
                      <div className="flex items-center gap-1.5">
                        <p className={`text-sm font-bold ${sel ? "text-brand-orange" : "text-gray-700"}`}>{dt.label}</p>
                        {sel && <span className="w-1.5 h-1.5 rounded-full bg-brand-orange inline-block" />}
                      </div>
                      <p className="text-xs text-brand-blue font-bold mt-0.5">{dt.pricePerKg} MAD/kg</p>
          ...
          {/* CALCUL DU PRIX */}
          <Section icon={Calculator} title="Calcul du Prix (Automatique)" badge="Auto">
            <div className="bg-gray-50 rounded-xl p-4 space-y-2">
              <div className="flex justify-between text-sm text-gray-600">
                <span>Type sélectionné</span>
                <span className="font-semibold text-gray-800">{deliveryType === "domicile" ? "À domicile" : "À l'agence"}</span>
              </div>
              <div className="flex justify-between text-sm text-gray-600">
                <span>Prix unitaire</span>
                <span className="font-semibold text-gray-800">{pricePerKg} MAD/kg</span>
              </div>
              <hr className="border-gray-200" />
              <div className="flex justify-between text-sm text-gray-600">
                <span>Poids du colis</span>
                <span className="font-semibold text-gray-800">{weight !== "" ? `${weight} kg` : "— kg"}</span>
              </div>
              <div className="flex justify-between text-sm text-gray-600">
              ...
              <div className="flex justify-between items-center pt-1">
                <span className="text-sm font-bold text-gray-700 uppercase tracking-wider">Prix Total</span>
                <span className={`text-2xl font-black ${totalPrice > 0 ? "text-brand-orange" : "text-gray-300"}`}>
            ...
            <div className="flex items-center gap-1.5 text-[11px] text-gray-400 mt-2">
              ...
              <span>Le prix se recalcule automatiquement si vous changez le type ou le poids.</span>
            </div>
          </Section>
          ...
          <Section icon={FileText} title="Notes">
            <textarea
              ...
              placeholder="Livraison rapide souhaitée, colis fragile, instructions spéciales..."
              rows={2}
      ...
      <div className="flex items-center justify-between py-4 border-t border-gray-200 bg-white sticky bottom-0">
        ...
        <div className="flex items-center gap-2">
          ...
          >
            <Printer size={15} /> Enregistrer & Imprimer
          </button>
          <button
            onClick={handleSave}
            disabled={!isValid}
